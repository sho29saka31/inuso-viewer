...
## viewer が「サービスを停止中です」のまま戻らない

### 症状
全ページがメンテナンス画面に切り替わり、戻らない。

### 原因と対処

`config/viewer_features` の `service` フィールドが `false` になっている。

1. DB管理者画面 `/db/features` で「Viewer を公開する」を ON にする
2. 変更保存後、ISR キャッシュが即時無効化され次のアクセス時に反映される
