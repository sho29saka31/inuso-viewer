...
const sections = [
  ...
  },
  {
    ...
    body: "本アプリはVercel（米Vercel Inc.）のホスティングを利用しており、アクセスログ（IPアドレス・ブラウザ情報等）がVercelのサーバーに記録される場合があります。またFirebase（Google LLC）を利用しており、データ通信がGoogleのサーバーを経由します。Cookie同意をいただいた利用者のアクセス情報はGoogle Analytics（Google LLC）に送信され、利用状況の把握に使用します。さらにエラー監視のためSentry（米Functional Software, Inc.）を利用しており、アプリ上で発生した技術的なエラー情報（IPアドレス・ブラウザ情報・スタックトレース等）がSentryのサーバーに送信される場合があります。これらの情報の取り扱いについては各社のプライバシーポリシーに従います。",
  },
  {
    title: "6. 未成年者の利用",
